...
static void mpt_scsi_init(MptState *s, PCIDevice *dev, Error **errp)
{
...
static void mpt_scsi_spi_init(PCIDevice *dev, Error **errp)
{
    MptState *s = PCI_LSI(dev);

    s->ctrl_type = MPTCTRLTYPE_SCSI_SPI;
    mpt_scsi_init(s, dev, errp);
}

static void mpt_scsi_sas_init(PCIDevice *dev, Error **errp)
{
    MptState *s = PCI_SAS(dev);

    s->ctrl_type = MPTCTRLTYPE_SCSI_SAS;
    mpt_scsi_init(s, dev, errp);
}

static void mpt_scsi_sase_init(PCIDevice *dev, Error **errp)
{
    MptState *s = PCI_SAS_E(dev);
